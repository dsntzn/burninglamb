...
const BigButton = ({ href = 'mailto:hi@burninglamb.eu', title, className, id, ...props }) => (
  <div id={id}>
    <a href={href} className={twMerge("relative group border-y-0.5 border-y-transparent hover:border-y-white transition-colors grid grid-cols-[1fr_15%] sm:grid-cols-[1fr_10%] md:grid-cols-[1fr_8%] flex-nowrap items-center lg:items-baseline justify-between text-black hover:text-white duration-500 py-8 px-4 md:px-0 w-full", className)} {...props} >
      ...
      <span className="absolute inset-x-0 md:-inset-x-[30px] inset-y-0 z-[-1] bg-primary-500 group-hover:bg-transparent transition-colors duration-500"></span>
    </a>
  </div>
)
